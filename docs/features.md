...
### 🧩 Vite + Pre-bundled Vendor
- **Vite 기반 개발**: 빠른 HMR(Hot Module Replacement)과 최적화된 프로덕션 빌드를 지원합니다.
- **Vendor Pre-bundling**: 무거운 외부 라이브러리(CodeMirror, Mermaid 등)는 미리 번들링하여 초기 로딩 속도를 최적화했습니다.
...
- **Inbox & Trash**: 분류되지 않은 노트용 Inbox와 안전한 삭제를 위한 휴지통(Tombstone 방식) 시스템을 제공합니다. Trash에는 노트 생성이 불가능하며, 빈 휴지통 기능이 컨텍스트 메뉴로도 제공됩니다.
...
### ✍️ 고도의 편집 기능 (Draftly / CodeMirror 6 기반)
...
- **Draftly 테이블 렌더링**:
  - 마크다운 표를 Draftly 위젯으로 렌더링합니다.
  - 별도 모달 없이 인라인 상호작용 중심으로 표를 다룹니다.
- **슬래시(`/`) 커맨드 및 자동 완성**:
...
- **최근 작업 노트 (Recent Notes)**:
  - **빠른 접근**: 사이드바 하단에 최근에 열어본 노트 10개를 표시합니다.
  - **맥락 표시**: 각 노트 항목에 폴더명을 함께 표기해 같은 제목의 노트도 구분하기 쉽습니다.
...
## 6. 외부 공유 API (External Sharing API)

### 🔌 API Key 기반 외부 연동
- **API Key 관리**: 설정 화면에서 API Key를 생성/삭제/복사할 수 있습니다.
- **REST API 제공**: 외부에서 노트를 조회, 수정, 검색, 목록할 수 있는 REST API를 제공합니다.
- **기능 지원**:
  - **목록 (GET /api/notes)**: 최신 수정일순으로 모든 노트 조회
  - **조회 (GET /api/notes/{id})**: 특정 노트 상세 조회
  - **생성 (POST /api/notes)**: Bearer API Key로 새 노트 생성
  - **수정 (PUT /api/notes/{id})**: 기존 노트 수정
  - **검색 (GET /api/notes?q=)**: 제목/내용 기반 검색
- **자세한 사용법**: [Skills API 문서](skills_api.md)를 참고하세요.

### 📑 멀티 탭 작업 환경 (Tab System)
- **탭 기반 멀티 태스킹**: 여러 노트를 동시에 탭으로 열어 빠르게 전환할 수 있습니다.
- **탭 관리 기능**:
  - 탭별 개별 닫기 버튼
  - 현재 탭 제외 전체 닫기 (`Close Other Tabs`)
  - 전체 탭 닫기 (`Close All Tabs`)
- **상태 복원**: 마지막으로 열린 탭(최대 10개)과 활성 탭이 자동으로 `localStorage`에 저장되고 복원됩니다.

---

## 8. 프로젝트 진화 이력 (Evolution Milestones)

- **Phase 1**: 마크다운 렌더링 및 클라우드 연동 기초 구축.
- **Phase 2**: Draftly(CodeMirror 6 기반) 에디터로 전환 및 UI 고밀도화.
...
## 9. 전문가용 네비게이션 및 인터페이스 (Modern Workspace)

### 🚀 활동 표시줄 (Activity Bar)
- **중앙 제어 허브**: 좌측 44px 너비의 고정 바를 통해 탐색기, 검색, 명령 팔레트, 퀵 노트를 즉시 실행합니다.
- **상시 접근성**: 모바일과 PC 모두에서 사이드바 패널의 열림 상태와 관계없이 주요 기능에 상시 접근할 수 있습니다.
- **지능형 전환**: 동일 아이콘 재클릭 시 사이드바를 접는 직관적인 토글 방식을 사용합니다.
- **메인 패널 연동 토글**: 메인 패널 좌측 헤더에도 사이드바 토글 버튼(`left_panel_open` / `left_panel_close`)이 제공됩니다.
  - **Desktop**: `isSidebarOpen`을 토글하여 사이드바 표시/숨김 전환
  - **Mobile**: `Explorer` 패널을 즉시 열어 사이드바로 복귀

### 🔍 스마트 검색 시스템
- **사이드바 검색 전용 패널**: Explorer와 분리된 검색 전용 공간에서 더 넓은 검색 결과와 스니펫을 확인할 수 있습니다.
- **검색 이력 관리**: 최근 검색어 10개를 자동으로 저장하며, 드롭다운(사이드바) 및 화살표 키(에디터) 탐색을 지원합니다.
- **자동 포커스**: 검색 패널 진입 시 즉시 타이핑이 가능하도록 입력창에 자동 포커스를 부여합니다.

### 📐 가변형 레이아웃
- **너비 조절 사이드바**: PC 환경에서 사이드바의 너비를 마우스 드래그로 자유롭게 조정할 수 있습니다 (최소 200px ~ 최대 600px).
- **레이아웃 보존**: 조정된 사이드바 너비와 열림/닫힘 상태는 브라우저를 껐다 켜도 완벽하게 유지됩니다.
- **반응형 최적화**: iOS 노치(Safe Area) 영역을 감지하여 모바일에서도 어긋남 없는 풀스크린 경험을 제공합니다.
