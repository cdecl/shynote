...
SHYNOTE는 **Draftly (CodeMirror 6 기반)** 마크다운 에디터를 탑재하여, 개발자에게 친숙한 VS Code 스타일의 경험을 웹에서 제공합니다.

## 1. 주요 편집 기능 (Core Features)

### 뷰 모드 (View Modes)
- **Live**: 편집과 렌더링이 동시에 반영되는 기본 모드.
- **View**: 마크다운 렌더링 결과를 확인하는 읽기 전용 모드.
  - **토글 인터페이스**: `Live` / `View` 버튼으로 즉시 전환할 수 있습니다.
...
- **선택 영역 가독성**: 다크 모드에서 선택된 텍스트는 **노란색(Amber) 계열 하이라이트**로 표시되어 명확하게 식별됩니다.
