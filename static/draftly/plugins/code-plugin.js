...
import { toggleMarkdownStyle } from "../editor/index.js";
import { highlightCode } from "@lezer/highlight";
import { languages } from "@codemirror/language-data";
import { createWrapSelectionInputHandler } from "../lib.js";
import { codePluginTheme as theme } from "./code-plugin.theme.js";
...
class CodePlugin extends DecorationPlugin {
  constructor() {
    super(...arguments);
    this.name = "code";
    this.version = "1.0.0";
    this.decorationPriority = 25;
    this.requiredNodes = ["InlineCode", "FencedCode", "CodeMark", "CodeInfo", "CodeText"];
    this.parserCache = /* @__PURE__ */ new Map();
  }
  /**
   * Plugin theme
   */
  ...
  async highlightCodeLines(code, lang, syntaxHighlighters) {
    ...
    try {
      ...
      highlightCode(
        ...
        (text, classes) => {
          const chunk = classes ? `<span class="${this.escapeAttribute(classes)}">${this.escapeHtml(text)}</span>` : this.escapeHtml(text);
