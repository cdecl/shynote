## [0.6.6] - 2026-01-19
### 🚀 주요 기능
- **사이드바 너비 조절**: PC 버전에서 마우스 드래그로 사이드바 너비를 자유롭게 조절하는 기능 추가 (설정값 자동 저장)

### 🔧 개선 사항
- **서비스 워커 업데이트 자동화**: 새 버전 배포 시 브라우저가 이를 감지하여 자동으로 페이지를 새로고침하는 로직 추가 (사용자 수동 새로고침 불편 해소)
- **iOS 레이아웃 최적화**: 아이폰 노치 영역 대응을 위해 중복 적용되었던 여백을 제거하여 사이드바와 메인 패널의 높이 일치
- **UI 레이블 개선**: 사이드바의 폴더 섹션 레이블을 "NEW"에서 "LIBRARY"로 변경하여 의미를 더 명확하게 개선
...
## [0.6.0] - 2026-01-17
### 🚀 주요 기능
- **게스트 모드**: 로그인 없이 앱 사용 가능 (자동 게스트 세션 생성)
- **인증 UI 개편**: 사이드바에 LOGIN/LOGOUT 버튼, Footer에 상태 표시 추가
- **백업/복원 개선**: 업소트(Upsert) 로직으로 중복 데이터 처리 및 멱등성 보장

### 🔧 개선 사항
- **복원 로직**: 폴더 먼저 복원 후 노트 처리로 folder_id 참조 문제 해결
- **다른 사용자 백업 차단**: backup_user_id 검증으로 타사용자 백업 복원 방지

### 🐛 버그 수정
- **복원 시 folder_id NULL 문제**: 폴더 flush/commit 후 노트 복원으로 해결

---

## [0.5.11] - 2026-01-15
### 🚀 주요 기능
- **에디터 툴바 확장**: 인용 부호(Blockquote) 버튼 추가로 마크다운 편집 기능 강화
- **미리보기 모드 개선**: 더블클릭으로 편집 모드 전환 및 시각적 힌트 시스템 도입
- **공용 알림 시스템**: 재사용 가능한 `showSignpost()` 함수로 사용자 가이드 메시지 표준화

### 🔧 개선 사항
- **블록 인용 지원**: `format_quote` 아이콘 추가 및 라인 기반 토글 기능 구현
- **더블클릭 이벤트**: 프리뷰 콘텐츠 더블클릭 시 에디터 모드로 자동 전환
- **시그널 포스트 시스템**: `showSignpost(text, duration, opacity)` 함수로 동적 알림 표시
- **투명도 조절**: 0.0~1.0 범위에서 투명도 조절 및 개행 문자 지원
