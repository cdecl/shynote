import { s as P, d as N, p as W } from "./styles-ca3715f6-2LloY4h8.js";
import { g as t, d as M, l as b, e as R, k as T, D as v, H as U, u as C } from "./vendor-Dsy1-eqI.js";
import { G as F } from "./graph-vxD8NXqM.js";
import { l as $ } from "./layout-CKZmpKt4.js";
const O = (e) => e.append("circle").attr("class", "start-state").attr("r", t().state.sizeUnit).attr("cx", t().state.padding + t().state.sizeUnit).attr("cy", t().state.padding + t().state.sizeUnit), X = (e) => e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", t().state.textHeight).attr("class", "divider").attr("x2", t().state.textHeight * 2).attr("y1", 0).attr("y2", 0), J = (e, i) => {
  ...
  const o = function(l, B, w) {
    const E = l.append("tspan").attr("x", 2 * t().state.padding).text(B);
  ...
  i.doc, l = a - c, x > p && (l = (p - d) / 2 + c), Math.abs(a - B.x) < c && x > p && (l = a - (x - p) / 2);
  ...
  return e.insert("rect", ":first-child").attr("x", l).attr("y", w).attr("class", o ? "alt-composit" : "composit").attr("width", d).attr(
    ...
    "y",
    t().state.titleShift - t().state.textHeight - t().state.padding
  ).attr("width", d).attr("height", B.height + 3 + 2 * t().state.textHeight).attr("rx", t().state.radius), e;
  ...
  i.type === "start" && O(s), i.type === "end" && _(s), (i.type === "fork" || i.type === "join") && q(s, i), i.type === "note" && D(i.note.text, s), i.type === "divider" && X(s), i.type === "default" && i.descriptions.length === 0 && J(s, i), i.type === "default" && i.descriptions.length > 0 && Y(s, i);
...
const K = function(e, i, o) {
  ...
  const s = i.points, g = v().x(function(r) {
    return r.x;
  }).y(function(r) {
    return r.y;
  ...
  if (t().state.arrowMarkerAbsolute && (a = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, a = a.replace(/\(/g, "\\("), a = a.replace(/\)/g, "\\)")), p.attr(
    ...
    for (let u = 0; u <= d.length; u++) {
      ...
      w = Math.max(w, f.width), E = Math.min(E, f.x), b.info(f.x, y, x + l), l === 0 && (l = h.node().getBBox().height, b.info("Title height", l, x)), B.push(h);
    }
    let k = l * d.length;
    if (d.length > 1) {
      const u = (d.length - 1) * l * 0.5;
      B.forEach((h, f) => h.attr("y", x + f * l - u)), k = l * d.length;
    }
...
const z = {}, Q = function() {
  ...
  let g;
  s === "sandbox" && (g = M("#i" + i));
  const p = s === "sandbox" ? M(g.nodes()[0].contentDocument.body) : M("body"), a = s === "sandbox" ? g.nodes()[0].contentDocument : document;
  b.debug("Rendering diagram " + e);
  const r = p.select(`[id='${i}']`);
  V(r);
  const y = c.db.getRootDoc();
  A(y, r, void 0, !1, p, a, c);
  const x = m.padding, d = r.node().getBBox(), l = d.width + x * 2, B = d.height + x * 2, w = l * 1.75;
  R(r, B, w, m.useMaxWidth), r.attr(
    "viewBox",
    `${d.x - m.padding}  ${d.y - m.padding} ` + l + " " + B
  );
}, tt = (e) => e ? e.length * m.fontSizeFactor : 1, A = (e, i, o, c, s, g, p) => {
  const a = new F({
    compound: !0,
    multigraph: !0
  });
  ...
  o ? a.setGraph({
    ...
    // acyclicer: 'greedy',
    ranker: "tight-tree",
    ranksep: y ? 1 : m.edgeLengthFactor,
    nodeSep: y ? 1 : 50,
    isMultiGraph: !0
    // ranksep: 5,
    // nodesep: 1
  ...
  for (const n of l) {
    ...
    if (u.doc) {
      ...
      h = A(u.doc, f, u.id, !c, s, g, p);
      {
        ...
        h.width = S.width, h.height = S.height + m.padding / 2, z[u.id] = { y: m.compositTitleSize };
      }
    } else
      h = L(i, u);
    if (u.note) {
  ...
  d.forEach(function(n) {
    B++, b.debug("Setting edge", n), a.setEdge(
      n.id1,
      n.id2,
      {
        relation: n,
        width: tt(n.title),
        height: m.labelHeight * T.getRows(n.title).length,
        labelpos: "c"
      },
      "id" + B
    );
  }), $(a), b.debug("Graph after layout", a.nodes());
  const w = i.node();
  a.nodes().forEach(function(n) {
    n !== void 0 && a.node(n) !== void 0 ? (b.warn("Node " + n + ": " + JSON.stringify(a.node(n))), s.select("#" + w.id + " #" + n).attr(
      ...
      let S = 0, H = 0;
      f && (f.parentElement && (S = f.parentElement.getBBox().width), H = parseInt(f.getAttribute("data-x-shift"), 10), Number.isNaN(H) && (H = 0)), h.setAttribute("x1", 0 - H + 8), h.setAttribute("x2", S - H - 8);
  ...
  return k.width = E.width + 2 * m.padding, k.height = E.height + 2 * m.padding, b.debug("Doc rendered", k, a), k;
}, et = {
  setConf: Q,
  draw: j
}, rt = {
