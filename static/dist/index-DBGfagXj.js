import { bb as G, bh as z, be as N, bp as I, bg as p, bt as Y, ba as j, b9 as U, bk as A, bv as Z, bc as B, bd as D, by as M } from "./vendor-DsVEk2JX.js";
...
function ge(e, t) {
  var o;
  let O = A(e).resolveInner(t, -1), r = null;
  ...
  if (r && (r.to > t || r.lastChild.type.isError)) {
    let a = r.parent;
    if (O.name == "TagName")
      return r.name == "CloseTag" || r.name == "MismatchedCloseTag" ? { type: "closeTag", from: O.from, context: a } : { type: "openTag", from: O.from, context: b(a) };
    ...
    return (s == null ? void 0 : s.name) == "StartTag" ? { type: "openTag", from: t, context: b(a) } : (s == null ? void 0 : s.name) == "StartCloseTag" && s.to <= t ? { type: "closeTag", from: t, context: a } : (s == null ? void 0 : s.name) == "Is" ? { type: "attrValue", from: t, context: r } : s ? { type: "attrName", from: t, context: r } : null;
  ...
  for (; O.parent && O.to == t && !(!((o = O.lastChild) === null || o === void 0) && o.type.isError); )
    O = O.parent;
  return O.name == "Element" || O.name == "Text" || O.name == "Document" ? { type: "tag", from: t, context: O.name == "Element" ? O : b(O) } : null;
}
class ue {
  constructor(t, o, O) {
...
const fe = /* @__PURE__ */ Y.inputHandler.of((e, t, o, O, r) => {
  ...
  let a = r(), { state: s } = a, f = s.changeByRange((n) => {
    var m, c, u;
    let { head: l } = n, d = s.doc.sliceString(l - 1, l) == O, S = A(s).resolveInner(l, -1), i;
    if (d && O == ">" && S.name == "EndTag") {
      let $ = S.parent;
      if (((c = (m = $.parent) === null || m === void 0 ? void 0 : m.lastChild) === null || c === void 0 ? void 0 : c.name) != "CloseTag" && (i = k(s.doc, $.parent, l))) {
        let g = l + (s.doc.sliceString(l, l + 1) === ">" ? 1 : 0), q = `</${i}>`;
        return { range: n, changes: { from: l, to: g, insert: q } };
      }
    } else if (d && O == "/" && S.name == "StartCloseTag") {
      let $ = S.parent;
      if (S.from == l - 2 && ((u = $.lastChild) === null || u === void 0 ? void 0 : u.name) != "CloseTag" && (i = k(s.doc, $, l))) {
        let g = l + (s.doc.sliceString(l, l + 1) === ">" ? 1 : 0), q = `${i}>`;
        return {
