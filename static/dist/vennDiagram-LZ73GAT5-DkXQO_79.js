import { bP as Gt, aD as w, aE as Wt, aF as Kt, aY as Ht, aX as Yt, aG as Xt, aH as Zt, b7 as wt, ba as Qt, d as ot, by as Jt, Q as $t, R as te, S as ee, aK as ne, b2 as se, b6 as ie, b8 as oe } from "./vendor-Dsy1-eqI.js";
...
function zt(t, n, s) {
  ...
  for (let v = 0; v < e; ++v) {
    ...
    const l = x[b];
    if (Q(S, 1 + h, E, -h, l), S.fx = t(S), S.fx < x[0].fx)
      Q(m, 1 + u, E, -u, l), m.fx = t(m), m.fx < S.fx ? M(m) : M(S);
    else if (S.fx >= x[b - 1].fx) {
      let p = !1;
      if (S.fx > l.fx ? (Q(g, 1 + r, E, -r, l), g.fx = t(g), g.fx < l.fx ? M(g) : p = !0) : (Q(g, 1 - r * h, E, r * h, l), g.fx = t(g), g.fx < S.fx ? M(g) : p = !0), p) {
        if (y >= 1) break;
        for (let f = 1; f < x.length; ++f)
          Q(x[f], 1 - y, x[0], y, x[f]), x[f].fx = t(x[f]);
      }
    } else
      M(S);
  }
...
function ce(t, n, s, e, i, o, a) {
  ...
  function b(x, M, A) {
    for (let E = 0; E < 16; ++E)
      if (i = (x + M) / 2, Q(e.x, 1, s.x, i, n), u = e.fx = t(e.x, e.fxprime), y = $(e.fxprime, n), u > c + o * i * h || u >= A)
        M = i;
      else {
        if (Math.abs(y) <= -a * h)
          return i;
  ...
  for (let x = 0; x < 10; ++x) {
    if (Q(e.x, 1, s.x, i, n), u = e.fx = t(e.x, e.fxprime), y = $(e.fxprime, n), u > c + o * i * h || x && u >= r)
      return b(d, i, r);
    if (Math.abs(y) <= -a * h)
      return i;
    if (y >= 0)
...
function ue(t, n, s) {
  ...
  for (let r = 0; r < u; ++r) {
    if (h = ce(t, a, e, i, h), s.history && s.history.push({
      ...
      Q(a, d, a, -1, i.fxprime), c = e, e = i, i = c;
    }
...
function be(t = {}) {
  ...
  function l(p) {
    ...
    if (f.length > 0) {
      ...
      c && (k = Ct(k, a, d)), I = Nt(k, s, e, i, h), D = Pt(I, f, M);
    }
    const U = {};
    f.forEach((k) => {
    ...
    ).attr("data-venn-sets", (k) => k.sets.join("_")), B = O.append("path"), W = O.append("text").attr("class", "label").text((k) => P(k)).attr("text-anchor", "middle").attr("dy", ".35em").attr("x", s / 2).attr("y", e / 2);
    ...
    let z = p;
    _ && typeof z.transition == "function" ? (z = K(p), z.selectAll("path").attrTween("d", j)) : z.selectAll("path").attr("d", (k) => St(k.sets.map((F) => I[F])), x);
    const L = z.selectAll("text").filter((k) => k.sets in D).text((k) => P(k)).attr("x", (k) => Math.floor(D[k.sets].x)).attr("y", (k) => Math.floor(D[k.sets].y));
    u && (_ ? "on" in L ? L.on("end", rt(I, P)) : L.each("end", rt(I, P)) : L.each(rt(I, P)));
    const C = K(V.exit()).remove();
    typeof V.transition == "function" && C.selectAll("path").attrTween("d", j);
    const X = C.selectAll("text").attr("x", s / 2).attr("y", e / 2);
    return y !== null && (W.style("font-size", "0px"), L.style("font-size", y), X.style("font-size", "0px")), { circles: I, textCentres: D, nodes: V, enter: O, update: z, exit: C };
  }
...
function Me(t, n = {}) {
  ...
  i && (x = Ct(x, o, a));
  const M = Nt(x, c, h, u, r), A = Pt(M, t, y), E = new Map(
    Object.keys(M).map((m) => [
      m,
      {
        set: m,
    ...
    const v = m.sets.map((f) => E.get(f)), l = jt(v), p = Vt(l, b);
...
var gt = (function() {
  var t = /* @__PURE__ */ w(function(S, g, m, v) {
    ...
    parse: /* @__PURE__ */ w(function(g) {
      var m = this, v = [0], l = [], p = [null], f = [], N = this.table, I = "", D = 0, U = 0, P = 2, T = 1, R = f.slice.call(arguments, 1), _ = Object.create(this.lexer), j = { yy: {} };
      for (var V in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, V) && (j.yy[V] = this.yy[V]);
      _.setInput(g, j.yy), j.yy.lexer = _, j.yy.parser = this, typeof _.yylloc > "u" && (_.yylloc = {});
      var O = _.yylloc;
      f.push(O);
      ...
      for (var z, L, C, X, k = {}, F, H, et, Y; ; ) {
        if (L = v[v.length - 1], this.defaultActions[L] ? C = this.defaultActions[L] : ((z === null || typeof z > "u") && (z = K()), C = N[L] && N[L][z]), typeof C > "u" || !C.length || !C[0]) {
          ...
          Y = [];
          for (F in N[L])
            this.terminals_[F] && F > P && Y.push("'" + this.terminals_[F] + "'");
          _.showPosition ? Z = "Parse error on line " + (D + 1) + `:
` + _.showPosition() + `
Expecting ` + Y.join(", ") + ", got '" + (this.terminals_[z] || z) + "'" : Z = "Parse error on line " + (D + 1) + ": Unexpected " + (z == T ? "end of input" : "'" + (this.terminals_[z] || z) + "'"), this.parseError(Z, {
            text: _.match,
...
var Be = /* @__PURE__ */ w((t, n, s, e) => {
  const i = e.db, o = i.getConfig?.(), { themeVariables: a, look: c, handDrawnSeed: h } = wt(), u = c === "handDrawn", r = [
    ...
    a.venn7,
    a.venn8
  ].filter(Boolean), y = i.getDiagramTitle?.(), d = i.getSubsetData(), b = i.getTextData(), x = qt(i.getStyleData()), M = o?.width ?? 800, A = o?.height ?? 450, S = M / 1600, g = y ? 48 * S : 0, m = a.primaryTextColor ?? a.textColor, v = Qt(n);
  ...
  l.selectAll(".venn-circle").each(function(T, R) {
    ...
    const R = ot(this), j = J([...T.sets].sort()), V = x.get(j), O = V?.fill;
    if (O) {
  ...
  const U = v.append("g").attr("transform", `translate(0, ${g})`), P = l.select("svg").node();
  if (P && "childNodes" in P)
...
function Ut(t, n, s, e, i, o) {
  ...
  for (const [r, y] of u.entries()) {
    ...
    const g = Math.max(80 * i, E * 2 * 0.95), m = Math.max(60 * i, E * 2 * 0.95), p = (d.data.label && d.data.label.length > 0 ? Math.min(32 * i, E * 0.25) : 0) + (y.length <= 2 ? 30 * i : 0), f = b - g / 2, N = x - m / 2 + p, I = Math.max(1, Math.ceil(Math.sqrt(y.length))), D = Math.max(1, Math.ceil(y.length / I)), U = g / I, P = m / D;
    for (const [T, R] of y.entries()) {
      const _ = T % I, j = Math.floor(T / I), V = f + U * (_ + 0.5), O = N + P * (j + 0.5);
      a && S.append("rect").attr("class", "venn-text-debug-cell").attr("x", f + U * _).attr("y", N + P * j).attr("width", U).attr("height", P).attr("fill", "none").attr("stroke", "teal").attr("stroke-width", 1 * i).attr("stroke-dasharray", `${4 * i} ${3 * i}`);
      const B = U * 0.9, W = P * 0.9, K = S.append("foreignObject").attr("class", "venn-text-node-fo").attr("width", B).attr("height", W).attr("x", V - B / 2).attr("y", O - W / 2).attr("overflow", "visible"), z = o.get(R.id)?.color, L = K.append("xhtml:span").attr("class", "venn-text-node").style("display", "flex").style("width", "100%").style("height", "100%").style("white-space", "normal").style("align-items", "center").style("justify-content", "center").style("text-align", "center").style("overflow-wrap", "normal").style("word-break", "normal").text(R.label ?? R.id);
      z && L.style("color", z);
    }
  }
}
