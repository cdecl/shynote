import { a as yt, s as dt, b as ft, c as pt, o as gt, p as mt, g as I, d as H, e as xt, r as kt } from "./vendor-Dsy1-eqI.js";
import { a as _t, g as it, f as bt, d as vt } from "./svgDrawCommon-b86b1483-DMFYOd5q.js";
...
Expecting ` + B.join(", ") + ", got '" + (this.terminals_[_] || _) + "'" : q = "Parse error on line " + (R + 1) + ": Unexpected " + (_ == J ? "end of input" : "'" + (this.terminals_[_] || _) + "'"), this.parseError(q, {
        ...
        switch (b[0]) {
          case 1:
            ...
            break;
          case 2:
            if ($ = this.productions_[b[1]][1], A.$ = f[f.length - $], A._$ = {
              ...
              return O;
            $ && (a = a.slice(0, -1 * $ * 2), f = f.slice(0, -1 * $), l = l.slice(0, -1 * $)), a.push(this.productions_[b[1]][0]), f.push(A.$), l.push(A._$), K = w[a[a.length - 2]][a[a.length - 1]], a.push(K);
            break;
          case 3:
            return !0;
        }
...
const Nt = function(t, e, i) {
  ...
  et++, n.append("line").attr("id", "task" + et).attr("x1", o).attr("y1", e.y).attr("x2", o).attr("y2", 450).attr("class", "task-line").attr("stroke-width", "1px").attr("stroke-dasharray", "4 2").attr("stroke", "#666"), Ft(n, {
    cx: o,
    cy: 300 + (5 - e.score) * 30,
    score: e.score
  });
...
const j = I().journey, P = j.leftMargin, Ot = function(t, e, i, o) {
  ...
  let c;
  h === "sandbox" && (c = H("#i" + e));
  const d = h === "sandbox" ? H(c.nodes()[0].contentDocument.body) : H("body");
  v.init();
  const y = d.select("#" + e);
  L.initGraphics(y);
...
}, W = j.sectionFills, st = j.sectionColours, qt = function(t, e, i) {
  const o = I().journey;
  let n = "";
  ...
  for (const [p, s] of e.entries()) {
    if (n !== s.section) {
      y = W[d % W.length], g = d % W.length, m = st[d % st.length];
      let a = 0;
      const u = s.section;
    ...
    s.x = p * o.taskMargin + p * o.width + P, s.y = c, s.width = o.diagramMarginX, s.height = o.diagramMarginY, s.colour = m, s.fill = y, s.num = g, s.actors = r, L.drawTask(t, s, o), v.insert(s.x, s.y, s.x + s.width + o.taskMargin, 450);
  }
}, nt = {
  setConf: zt,
  draw: Ot
