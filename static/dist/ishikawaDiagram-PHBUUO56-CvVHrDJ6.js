import { aD as o, aI as ot, bb as ut, ba as dt, by as yt, b2 as ft, aQ as pt, aX as it, aG as gt, aH as kt, aF as mt, aE as wt, aY as bt, aK as _t } from "./vendor-Dsy1-eqI.js";
var J = (function() {
  var e = /* @__PURE__ */ o(function(T, t, s, i) {
    for (s = s || {}, i = T.length; i--; s[T[i]] = t) ;
    return s;
  }, "o"), h = [1, 4], r = [1, 14], a = [1, 12], l = [1, 13], y = [6, 7, 8], f = [1, 20], d = [1, 18], m = [1, 19], u = [6, 7, 11], k = [1, 6, 13, 14], g = [1, 23], b = [1, 24], x = [1, 6, 7, 11, 13, 14], D = {
    ...
    parse: /* @__PURE__ */ o(function(t) {
      var s = this, i = [0], c = [], p = [null], n = [], v = this.table, w = "", I = 0, E = 0, L = 2, A = 1, C = n.slice.call(arguments, 1), _ = Object.create(this.lexer), S = { yy: {} };
      for (var P in this.yy)
        Object.prototype.hasOwnProperty.call(this.yy, P) && (S.yy[P] = this.yy[P]);
      _.setInput(t, S.yy), S.yy.lexer = _, S.yy.parser = this, typeof _.yylloc > "u" && (_.yylloc = {});
      var R = _.yylloc;
      n.push(R);
      var G = _.options && _.options.ranges;
      ...
      function tt() {
        var B;
        return B = c.pop() || _.lex() || A, typeof B != "number" && (B instanceof Array && (c = B, B = c.pop()), B = s.symbols_[B] || B), B;
      }
      o(tt, "lex");
      for (var M, F, N, K, W = {}, U, V, et, X; ; ) {
        if (F = i[i.length - 1], this.defaultActions[F] ? N = this.defaultActions[F] : ((M === null || typeof M > "u") && (M = tt()), N = v[F] && v[F][M]), typeof N > "u" || !N.length || !N[0]) {
          var Q = "";
          X = [];
          for (U in v[F])
            this.terminals_[U] && U > L && X.push("'" + this.terminals_[U] + "'");
          _.showPosition ? Q = "Parse error on line " + (I + 1) + `:
` + _.showPosition() + `
Expecting ` + X.join(", ") + ", got '" + (this.terminals_[M] || M) + "'" : Q = "Parse error on line " + (I + 1) + ": Unexpected " + (M == A ? "end of input" : "'" + (this.terminals_[M] || M) + "'"), this.parseError(Q, {
            text: _.match,
        ...
        if (N[0] instanceof Array && N.length > 1)
          throw new Error("Parse Error: multiple actions possible at state: " + F + ", token: " + M);
        switch (N[0]) {
          case 1:
            i.push(M), p.push(_.yytext), n.push(_.yylloc), i.push(N[1]), M = null, E = _.yyleng, w = _.yytext, I = _.yylineno, R = _.yylloc;
            break;
          case 2:
            if (V = this.productions_[N[1]][1], W.$ = p[p.length - V], W._$ = {
              ...
              return K;
            V && (i = i.slice(0, -1 * V * 2), p = p.slice(0, -1 * V), n = n.slice(0, -1 * V)), i.push(this.productions_[N[1]][0]), p.push(W.$), n.push(W._$), et = v[i[i.length - 2]][i[i.length - 1]], i.push(et);
            break;
          case 3:
            return !0;
        }
...
` + this.showPosition(), {
  ...
  return o($, "Parser"), $.prototype = D, D.Parser = $, new $();
})();
J.parser = J;
var xt = J, H, vt = (H = class {
...
}, o(H, "IshikawaDB"), H), St = 14, j = 250, Et = 30, $t = 60, At = 5, ht = 82 * Math.PI / 180, st = Math.cos(ht), nt = Math.sin(ht), at = /* @__PURE__ */ o((e, h, r) => {
  ...
  if (!y)
    return;
  const f = ot(), { look: d, handDrawnSeed: m, themeVariables: u } = f, k = ut(f.fontSize)[0] ?? St, g = d === "handDrawn", b = y.children ?? [], x = f.ishikawa?.diagramPadding ?? 20, D = f.ishikawa?.useMaxWidth ?? !1, O = dt(h), $ = O.append("g").attr("class", "ishikawa"), T = g ? yt.svg(O.node()) : void 0, t = T ? {
    roughSvg: T,
    seed: m ?? 0,
  ...
  let i = 0, c = j;
  const p = g ? void 0 : z($, i, c, i, c, "ishikawa-spine");
  if (Lt($, i, c, y.text, k, t), !b.length) {
    g && z($, i, c, i, c, "ishikawa-spine", t), at(O, x, D);
    return;
  }
  i -= 20;
  const n = b.filter((S, P) => P % 2 === 0), v = b.filter((S, P) => P % 2 === 1), w = rt(n), I = rt(v), E = w.total + I.total;
  let L = j, A = j;
  if (E > 0) {
    const S = j * 2, P = j * 0.3;
    L = Math.max(P, S * (w.total / E)), A = Math.max(P, S * (I.total / E));
  }
  const C = k * 2;
  L = Math.max(L, w.max * C), A = Math.max(A, I.max * C), c = Math.max(L, j), p && p.attr("y1", c).attr("y2", c), $.select(".ishikawa-head-group").attr("transform", `translate(0,${c})`);
  const _ = Math.ceil(b.length / 2);
  for (let S = 0; S < _; S++) {
    const P = $.append("g").attr("class", "ishikawa-pair");
    for (const [R, G, Z] of [
      [b[S * 2], -1, L],
      [b[S * 2 + 1], 1, A]
    ])
      R && Pt(P, R, i, c, G, Z, k, t);
  ...
  if (g)
    ...
    p.attr("x1", i);
    const S = `url(#${s})`;
    $.selectAll("line.ishikawa-branch, line.ishikawa-sub-branch").attr("marker-start", S);
  }
  at(O, x, D);
  ...
  const f = Math.max(6, Math.floor(110 / (l * 0.6))), d = e.append("g").attr("class", "ishikawa-head-group").attr("transform", `translate(${h},${r})`), m = Y(
    d,
    ct(a, f),
    0,
    0,
    ...
    "start",
    l
  ), u = m.node().getBBox(), k = Math.max(60, u.width + 6), g = Math.max(40, u.height * 2 + 40), b = `M 0 ${-g / 2} L 0 ${g / 2} Q ${k * 2.4} 0 0 ${-g / 2} Z`;
  if (y) {
    const x = y.roughSvg.path(b, {
      roughness: 1.5,
      seed: y.seed,
      fill: y.fillColor,
      fillStyle: "hachure",
    ...
    d.insert("path", ":first-child").attr("class", "ishikawa-head").attr("d", b);
  ...
  const d = e.append("g").attr("class", "ishikawa-label-group"), u = Y(
    d,
    h,
    r,
    a + 11 * l,
  ...
  if (f === 0)
    return;
  const d = a / f, m = l / f, u = 6, k = -m * u, g = d * u, b = h, x = r, D = `M ${b} ${x} L ${b - d * u * 2 + k} ${x - m * u * 2 + g} L ${b - d * u * 2 - k} ${x - m * u * 2 - g} Z`, O = y.roughSvg.path(D, {
    roughness: 1,
    seed: y.seed,
    fill: y.lineColor,
    fillStyle: "solid",
  ...
  const m = h.children ?? [], u = y * (m.length ? 1 : 0.2), k = -st * u, g = nt * u * l, b = r + k, x = a + g;
  if (z(e, r, a, b, x, "ishikawa-branch", d), d && q(e, r, a, r - b, a - x, d), Mt(e, h.text, b, x, l, f, d), !m.length)
    return;
  const { entries: D, yOrder: O } = Tt(m, l), $ = D.length, T = new Array($);
  ...
  for (const [p, n] of D.entries()) {
    ...
    if (n.depth % 2 === 0) {
      const C = w.y1 - w.y0;
      E = lt(w.x0, w.x1, C ? (v - w.y0) / C : 0.5), L = v, A = E - (n.childCount > 0 ? $t + n.childCount * At : Et), z(I, E, v, A, v, "ishikawa-sub-branch", d), d && q(I, E, v, 1, 0, d), Y(I, n.text, A, v, "ishikawa-label align", "end", f);
    } else {
      const C = w.childrenDrawn++;
      E = lt(w.x0, w.x1, (w.childCount - C) / (w.childCount + 1)), L = w.y0, A = E + s * ((v - L) / i), z(I, E, L, A, v, "ishikawa-sub-branch", d), d && q(I, E, L, E - A, L - v, d), Y(I, n.text, A, v, c, "end", f);
    }
  ...
  return r.join(`
`);
}, "wrapText"), Y = /* @__PURE__ */ o((e, h, r, a, l, y, f) => {
