...
type WrapMarkerMap = Record<string, string>;

/**
 * Creates an input handler that wraps selected text when a marker is typed.
 *
 * Example: typing "*" with a selection wraps it as *selected*.
 * Special case like "=" can map to "==" to wrap as ==selected==.
 */
export function createWrapSelectionInputHandler(markers: WrapMarkerMap) {
