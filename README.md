...
- **GitHub 스타일 마크다운 & Mermaid**: 
  - Preview 모드에서 GitHub Markdown CSS를 적용하여 전문적인 문서 렌더링을 제공합니다.
  - `mermaid` 코드 블록을 통해 차트와 다이어그램을 에디터 내에서 바로 렌더링할 수 있습니다.
- **Ultra-Compact UI**: 헤더 높이를 24px로 최소화하여 콘텐츠 영역을 극대화한 미니멀 디자인을 적용했습니다.
- **테마 및 개인화**: 
...
- **Slash Commands (`/`)**: 
  - `/` 키를 눌러 날짜, 시간 입력부터 테이블/코드 블록 삽입까지 키보드 하나로 해결합니다.
  - 동적 자동완성을 통해 문서 작성을 더욱 빠르게 가속화합니다.
...
- **Multi-Select & Bulk Actions**: `Cmd/Ctrl` 키로 여러 노트를 선택하여 한 번에 이동하거나 삭제할 수 있습니다.
...
- **휴지통 (Trash)**: 실수로 삭제한 노트와 폴더를 임시 보관하고 복구할 수 있는 안전장치를 제공합니다.
...
- **Google 로그인**: Google OAuth 2.0 (Redirect Flow)을 지원하여 안전하고 끊김 없는 로그인을 제공합니다.
- **게스트 모드 (Guest Mode)**: 
...
- **기능**: 키보드만으로 빠른 탐색 및 명령 실행
  ...
  - 글자 크기 조절, 강제 동기화
  - 캐시 초기화, 앱 새로고침
- **UX**: 키보드 네비게이션 완전 지원 및 화면 상단 최적화 배치
