...
SHYNOTE는 복잡한 설정 없이 사용자가 온전히 글쓰기에만 몰입할 수 있도록 설계된 경량 웹 기반 마크다운 노트 어플리케이션입니다. 이제 **Activity Bar**와 **Resizable Sidebar**를 통해 더 전문적인 작업 환경을 제공합니다.
...
### 1. 전문가급 네비게이션 (Professional Workspace)
- **액티비티 바(Activity Bar)**: VS Code 스타일의 슬림한 사이드바를 통해 탐색기, 검색, 설정을 즉시 전환합니다.
- **너비 조절 사이드바**: 작업 환경에 맞춰 마우스 드래그로 사이드바 너비를 자유롭게 조정할 수 있습니다 (PC).
- **스마트 검색 이력**: 최근 검색어 10개를 저장하고, 화살표 키나 드롭다운으로 빠르게 재사용합니다.
- **자동 업데이트**: 배포 즉시 브라우저가 새 버전을 감지하여 자동으로 최신화합니다.
- **멀티 탭 작업**: 여러 노트를 탭으로 동시에 열어 빠르게 전환하며, 마지막 상태가 자동으로 복원됩니다.

### 2. 강력한 편집 경험 (Advanced Editor)
...
- **외부 공유 API**: API Key를 생성하여 노트를 외부에서 조회, 수정, 검색, 목록할 수 있는 REST API를 제공합니다.
...
- **Build**: Vite + Pre-bundled Vendor (성능 최적화)
