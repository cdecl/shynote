...
async def google_oauth_callback(
    ...
    if not GOOGLE_CLIENT_SECRET:
        raise HTTPException(
            status_code=500, detail="Server config error: Missing Client Secret"
        )
    ...
    try:
        ...
        if not origin:
            raise HTTPException(
                status_code=400, detail="Could not determine origin for redirect_uri"
            )
...
def create_folder(
    ...
    existing_folder = (
        db.query(models.Folder).filter(models.Folder.id == folder.id).first()
    )
    ...
    if existing_folder:
        if existing_folder.user_id != current_user.id:
            ...
            print(
                f"Collision: Folder {folder.id} exists for user {existing_folder.user_id}, but requested by {current_user.id}"
            )
            raise HTTPException(
                status_code=409, detail="Folder ID collision with another user"
            )
...
def create_note(
    ...
    if existing_note:
        if existing_note.user_id != current_user.id:
            ...
            print(
                f"Collision: Note {note.id} exists for user {existing_note.user_id}, but requested by {current_user.id}"
            )
            ...
            raise HTTPException(
                status_code=409, detail="Note ID collision with another user"
            )
    ...
    if db_note is None:
        ...
        if note.version is not None:
            if db_note.version != note.version:
                print(
                    f"Conflict: Note {note_id} version mismatch. DB={db_note.version}, Client={note.version}"
                )
...
def backup_data(
    ...
    return {
        "folders": folders,
        "notes": notes,
        "backup_user_id": current_user.id  # Track who created this backup
    }
...
def restore_data(
    ...
    """
    Restores folders and notes from a backup file using upsert logic.
    Only allows restoring backups created by the same user.
    Idempotent: Safe to call multiple times with same data.
    """
    # Validate that the backup was created by this user
    backup_user_id = getattr(backup, 'backup_user_id', None)
    if backup_user_id and backup_user_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="This backup was created by another user and cannot be restored."
        )

    # Track seen IDs to handle duplicates within the backup file itself
    seen_folder_ids = set()
    seen_note_ids = set()
    ...
    for folder_data in backup.folders:
        # Skip duplicates within the backup file
        if folder_data.id in seen_folder_ids:
            continue
        seen_folder_ids.add(folder_data.id)

        existing = (
            db.query(models.Folder)
            .filter(
                models.Folder.id == folder_data.id,
                models.Folder.user_id == current_user.id,
            )
            .first()
        )
        ...
        if existing:
            ...
            new_folder = models.Folder(
                id=folder_data.id, name=folder_data.name, user_id=current_user.id
            )
            db.add(new_folder)
            folders_added += 1

    # Flush and commit folders before restoring notes
    # This ensures folder IDs are visible when restoring notes
    db.flush()
    db.commit()
    ...
    for note_data in backup.notes:
        # Skip duplicates within the backup file
        if note_data.id in seen_note_ids:
            continue
        seen_note_ids.add(note_data.id)

        existing = (
            db.query(models.Note)
            .filter(
                models.Note.id == note_data.id, models.Note.user_id == current_user.id
            )
            .first()
        )

        folder_id = note_data.folder_id
        # Note: folder_id is preserved as-is since folders are already restored above
        ...
        if existing:
            ...
            existing.is_pinned = note_data.is_pinned
            existing.is_shared = note_data.is_shared
            existing.share_id = note_data.share_id
            existing.version = note_data.version
            notes_updated += 1
        else:
...
def view_shared_note(share_id: str, db: Session = Depends(database.get_db)):
    note = (
        db.query(models.Note)
        .filter(models.Note.share_id == share_id, models.Note.is_shared == True)
        .first()
    )
